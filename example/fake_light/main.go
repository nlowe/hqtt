...
func main() {
	...
	a, ok := hassAvailability.Get()
	log.With(slog.String("state", string(a)), slog.Bool("ok", ok)).Info("Detected Home Assistant State")
	if !ok || a != hass.Available {
	...
	rediscover := func() error {
		...
		return d.Configure(ctx, w, discovery.DefaultPrefix)
	}
	...
	err = errors.Join(
		hqtt.AssociateComponentWith(l, d),
		hqtt.AssociateComponentWith(s, d),
		rediscover(),
	)
	if err != nil {
		panic(err)
