...
var (
	// ErrMissingUniqueID is the error returned by AssociateComponentWith if the provided Component is missing its
	// Component.UniqueID.
	ErrMissingUniqueID = errors.New("component must have a UniqueID")
	// ErrDuplicateComponent is the error returned by AssociateComponentWith when another Component with the same
	// Component.UniqueID has already been associated with this Device.
	...
	ErrInvalidDevice = errors.New("device must have at least one identifying value in 'identifiers' and/or 'connections'")
)
...
// Device represents an MQTT-based HomeAssistant device.
//
...
// AssociateComponentWith associates the specified Component with the device. This association must be setup prior to
// emitting the Device Discovery payload. The Component.UniqueID must be set, and be unique across all Components that
// are associated with the Device.
func AssociateComponentWith[T Platform](c Component[T], d *Device) error {
	if c.UniqueID == "" {
	...
	if d.entities == nil {
		d.entities = make(map[string]any)
	}

	if _, ok := d.entities[c.UniqueID]; ok {
		return fmt.Errorf("%s: %w", c.UniqueID, ErrDuplicateComponent)
	}

	d.entities[c.UniqueID] = c
	return nil
}

// ID calculates and identifier for this device. If the Device.DiscoveryID is specified, that value will be used.
...
// Configure updates the device discovery payload for this device and its associated entities. All Components must
// already be associated with the device via AssociateComponentWith before calling Configure. The device must pass
// validation performed by Device.Valid.
func (d *Device) Configure(ctx context.Context, w mqtt.Writer, discoveryPrefix string) error {
	// Validation
	...
	err := errors.Join(
		...
		discovery.MaybeInlineMarshalStd(e, d.entities),
